...
def secClassification(connexion, tab_ref, tab_out, thresholds, save_intermediate_results = False, debug = 0):
    ...
    tab_out = distinctForestLineTreeShrub(connexion, tab_out, thresholds["val_largeur_max_alignement"], save_intermediate_results, debug = debug)
  ...
  #  createConvexityIndicator(connexion, tab_out, debug = debug)
   ...
   # createExtensionIndicator(connexion,tab_out)
    ...
    WHERE rgt.id_elong  >= %s;
    """ %(tab_out, name_algt, thresholds["seuil_elongation"])

    query += """
    UPDATE %s AS rgt SET fv='%s'
    WHERE rgt.id_elong < %s;
    """ %(tab_out, name_bst, thresholds["seuil_elongation"])
...
########################################################################
def distinctForestLineTreeShrub(connexion, tab_rpgt, seuil_larg, save_intermediate_results= False, debug = 0):
    """
