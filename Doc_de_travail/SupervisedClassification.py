...
def classifySupervised(image_input_list, sample_points_values_input, classification_file_output, confidence_file_output, model_output, model_input, field_class, sampler_mode, periodic_value, classifier_mode, kernel, rf_parametres_struct, no_data_value, path_time_log, ram_otb=0,  format_raster='GTiff', extension_vector=".shp", save_results_intermediate=False, overwrite=True):
    ...
    if check and not overwrite:
        ...
        for image_input_tmp in image_input_list :
            ...
            command = "otbcli_ImageClassifier -in %s -imstat %s -model %s -out %s %s" %(image_input_tmp, statistics_image_input, model_file_output, classification_file_output_tmp, CODAGE_16B)
